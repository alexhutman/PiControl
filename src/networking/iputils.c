...
char *pictrl_get_ip_address() {
  ...
  case AF_INET:
    ...
    ip_query_res = getnameinfo(iface->ifa_addr, IPV4_SOCKADDR_SZ, ip, INET_ADDRSTRLEN, NULL, 0,
                               NI_NUMERICHOST);
    break;
  case AF_INET6:
    ip = malloc(INET6_ADDRSTRLEN);
    ip_query_res = getnameinfo(iface->ifa_addr, IPV6_SOCKADDR_SZ, ip, INET6_ADDRSTRLEN, NULL, 0,
                               NI_NUMERICHOST);
    break;
  default:
