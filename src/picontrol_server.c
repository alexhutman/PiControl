...
static int handle_connection(pictrl_client_t *pi_client, pictrl_rb_t *rb,
                             pictrl_backend *backend) {
  ssize_t n;
  // TODO: Block on this write, since we can't do anything unless we have the
  // command and payload_size
  while ((n = pictrl_rb_write(pi_client->connfd, 2, rb)) >
         0) {  // 2 for the `cmd` and `payload_size`
