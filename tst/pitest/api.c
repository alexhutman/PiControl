...
size_t run_test_suite(const TestSuite *suite) {
    ...
    int suite_setup_ret;
    if ((suite_setup_ret = run_setup_if_exists(suite_setup)) != 0) {
    ...
    for (size_t test_id = 0; test_id < suite->num_tests; test_id++) {
        ...
        int case_setup_ret;
        if ((case_setup_ret = run_setup_if_exists(case_setup)) != 0) {
        ...
        int case_teardown_ret;
        if ((case_teardown_ret = run_teardown_if_exists(case_teardown)) != 0) {
    ...
    int suite_teardown_ret;
    if ((suite_teardown_ret = run_teardown_if_exists(suite_teardown)) != 0) {
